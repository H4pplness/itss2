...
class BookPlayerTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final listInvitation = ref.watch(invitationNotifierProvider);
    final listBooking = ref.watch(myBookingPlayerNotifierProvider);
    return Container(
                        ...
                        Row(
                          ...
                          children: [
                            TextButton(
                                onPressed: () async {
                                  if (status == 1) {
                                    await ref.read(updateInvitationProvider(
                                        UpdateInvitationParam(
                                            invitationId: invitation.id,
                                            status: 'accepted')));
                                    ref.read(invitationNotifierProvider.notifier).changeStatus(listInvitation.indexOf(invitation), 'accepted');
                                  ...
                                  if (status == 1) {
                                    await ref.read(updateInvitationProvider(
                                        UpdateInvitationParam(
                                            invitationId: invitation.id,
                                            status: 'rejected')));
                                    ref.read(invitationNotifierProvider.notifier).changeStatus(listInvitation.indexOf(invitation), 'rejected');
                                  }
                                },
                                child: Text("Reject",
                                    style: GoogleFonts.montserrat(
